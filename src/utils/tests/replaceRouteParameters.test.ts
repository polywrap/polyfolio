...
test('Replace route parameters test: many parameters', () => {
  const result = replaceRouteParameters(RoutePath.Asset, {
    user: '0x5f06596lwejflsfer',
    chainId: '1',
    asset: 'fwb',
  });
  const result_alt = replaceRouteParameters(RoutePath.Asset, {
...
test('Replace route parameters test: parameter collision', () => {
  const result = replaceRouteParameters(RoutePath.Dashboard, {
    user: '0x5f06596lwejflsfer',
