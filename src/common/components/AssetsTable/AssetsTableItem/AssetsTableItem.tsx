...
import {fillArray, toFixed} from 'utils/helpers';
    ...
    <>
      <button className={styles.buttonNavigate} onClick={() => navigate(path)}>
        <div className={styles.menu_item}>
          ...
          <PricesValue
            secondaryPricePercentTitle={toFixed({value: secondaryPricePercentTitle ?? 0, size: 2})}
            pricePercentDollar={toFixed({value: pricePercentDollar ?? 0, size: 2})}
            className={classNames(styles.price_container, {
              [styles.hidden]: filters.assets.price,
            })}
            valueIsMinus={valueIsMinus}
            priceTitle={toFixed({value: priceTitle ?? 0, size: 2})}
          />
          <div
          ...
          >
            <div>
              <div className={styles.valueTitle}>
                ${toFixed({value: valueTitle, size: 2})}
              </div>
              <div className={styles.valueSecondaryContainer}>
                ${toFixed({value: valueSecondaryTitle, size: 2})}
